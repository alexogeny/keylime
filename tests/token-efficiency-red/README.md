...
| `research-theses.red.test.ts` | `extensions/shared/tool-result-economics.ts`, typed reducers | Deterministic token estimates, recoverable break-even decisions, cache-separated savings, and task-conditioned exact-line selection without auxiliary model calls |
...
The initial suite discovered 25 tests, expanded to 77 roadmap contracts, then added seven production-hook wiring checks and four deterministic research-thesis contracts. The current baseline is **88 pass, 0 fail**. The files retain the `.red.test.ts` suffix as permanent regression contracts; empirical rollout evidence remains separate from contract completion.
