...
class BM25Index {
  ...
  remove(id: string): void {
    ...
    if (idx !== -1) { this.docs.splice(idx, 1); this.dirty = true; }
  }

  private _recompute(): void {
    if (!this.dirty) return;
    const N  = this.docs.length;
    if (N === 0) { this.avgLen = 0; this.idf.clear(); this.dirty = false; return; }
    ...
    for (const doc of this.docs) {
      ...
      for (const t of qt) {
        ...
        score += idf * (tf * (BM25_K1 + 1)) /
                 (tf + BM25_K1 * (1 - BM25_B + BM25_B * doc.len / (this.avgLen || 1)));
      }
...
// ─── TF-IDF Vector Store (pure-TS cosine similarity, zero deps) ───────────────
//
// We store a TF-IDF term vector per memory on write.  At query time we build
// a query vector and compute cosine similarity in O(unique query terms) per doc
// because the dot product only visits terms that appear in the query.
// At hundreds of memories this is <1ms.
...
class TFIDFStore {
  ...
  /** Recompute IDF across all stored vectors and return the IDF map. */
  private _idf(): Map<string, number> {
    const idf = new Map<string, number>();
    for (const [term, freq] of this.df) {
      idf.set(term, Math.log((this.N + 1) / (freq + 1)) + 1); // smoothed IDF
    }
    return idf;
  }

  /** Build a TF-IDF vector for a piece of text given current corpus stats. */
  ...
  add(id: string, text: string): void {
    ...
    this.N++;
    // Build and store vector (we'll lazily rebuild idf-weighted on query)
    const rawTF: Record<string, number> = {};
    for (const t of tokens) rawTF[t] = 1;
    this.vectors.set(id, rawTF); // stored as raw presence; weighted at query time
  }
  ...
  /** Cosine similarity between a query and a stored document. O(|query terms|). */
  ...
  /** Top-K cosine search across all stored vectors. */
  search(queryText: string, topK: number, candidateIds?: string[]): Array<{ id: string; score: number }> {
