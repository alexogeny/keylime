...
`clarification.ts` provides `/clarify <rough task>` and `/submit-clarified`. It deterministically infers task concepts, expands overloaded language (for example `↑`/“up tokens” into input-token, prompt-cache, and context-usage concepts), applies concept-specific path priors, retrieves bounded repository evidence, and separately recalls semantically relevant saved web research. It then makes at most one tool-free model call to turn those typed context sources into a structured execution prompt. `KEYLIME_CLARIFICATION_MODE=deterministic` disables the model pass and uses the local semantic fallback; tests evaluate retrieval against deliberately rough Keylime prompts without any model dependency.
