...
`clarification.ts` provides `/clarify <rough task>` and `/submit-clarified`. It uses domain-agnostic pseudo-relevance feedback: preliminary repository and saved-research matches produce salient multi-document phrases, those phrases expand a second retrieval pass, and generic source-role priors favor implementation evidence without encoding task-specific paths or vocabulary. When tentative or optimization-oriented wording is materially reframed by relevant saved research, `/clarify` asks whether to run fresh read-only research before producing a brief. It otherwise makes at most one tool-free model call over the typed evidence. `KEYLIME_CLARIFICATION_MODE=deterministic` disables that model pass; corpus tests contain rough examples but production ranking has no per-example semantic rules.
...
`git-checkpoint.ts` checkpoints at the end of every successful mutating agent turn by default, including small targeted edits, so completed work does not silently accumulate uncommitted. It asks Pi's active authenticated model for a semantic subject and body, validates the structured result, and falls back to a deterministic local message on timeout, malformed output, or missing model access. In TUI mode the default review dialog lets you approve, edit, or skip before anything is staged, and the footer status reports preparing, skipped, blocked, clean, or committed outcomes. Set `KEYLIME_AUTO_CHECKPOINT=major` for lower-noise major-only checkpoints, or `off` for manual-only checkpoints. It excludes `.pi` local state from staging. Manual `/checkpoint` is still available. `git-tools.ts` provides read-only `git_status`, `git_diff`, `commit_history`, `see_file_commit_history`, and `inspect_at_checkpoint` so agents do not need raw git inspection commands.
