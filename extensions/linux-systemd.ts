...
export default function (pi: ExtensionAPI) {
  pi.registerTool({ name: "systemd_list_units", label: "Systemd List Units", description: "List systemd units with bounded output.", promptGuidelines: guidelines, parameters: Type.Object({ state: Type.Optional(Type.String()) }), async execute(_id: string, params: any) {
  ...
  pi.registerTool({ name: "systemd_status", label: "Systemd Status", description: "Inspect one systemd unit status.", promptGuidelines: guidelines, parameters: unitParam, async execute(_id: string, params: any) {
  ...
  pi.registerTool({ name: "systemd_logs", label: "Systemd Logs", description: "Inspect bounded journal logs for one unit.", promptGuidelines: guidelines, parameters: Type.Object({ unit: Type.String(), since: Type.Optional(Type.String()), lines: Type.Optional(Type.Number({ minimum: 1, maximum: 500 })) }), async execute(_id: string, params: any) {
  ...
  pi.registerTool({ name: "systemd_plan_restart", label: "Systemd Plan Restart", description: "Validate that a unit restart is allowed and show the command.", promptGuidelines: guidelines, parameters: unitParam, async execute(_id: string, params: any) {
  ...
  for (const action of ["restart", "enable", "disable"] as const) {
    pi.registerTool({ name: `systemd_${action}`, label: `Systemd ${action}`, description: `${action} a systemd unit after review and sudo approval.`, promptGuidelines: [...guidelines, `Run systemd_plan_restart before restart; inspect status first for ${action}.`], parameters: unitParam, async execute(_id: string, params: any, _signal: any, _onUpdate: any, ctx: any) {
