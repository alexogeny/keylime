...
export function defaultCheckCommands(kind: ProjectKind, suite: CheckSuite): CheckCommand[] {
  ...
  if (kind === "typescript") {
    if (suite === "test") return [{ command: "bun", args: ["test", "tests/*.test.ts"], label: "bun test tests/*.test.ts" }];
    if (suite === "typecheck") return [{ command: "bun", args: ["--check", "extensions/**/*.ts", "tests/*.test.ts"], label: "bun --check" }];
    if (suite === "lint") return [];
    return [
      { command: "bun", args: ["test", "tests/*.test.ts"], label: "bun test tests/*.test.ts" },
      { command: "bun", args: ["--check", "extensions/shared/*.ts", "extensions/*.ts", "tests/*.test.ts"], label: "bun --check" },
    ];
  }
